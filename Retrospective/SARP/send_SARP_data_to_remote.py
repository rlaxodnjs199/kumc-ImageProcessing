...
def prepare_ProjSubjList(df_QCTCFD: pd.DataFrame, case_ids: List[int], index: int):
    ...
    for case_id in case_ids:
        ...
        if (df_QCTCFD["VIDA_IN_Path"].str.contains(str(case_id))).any():
            df_QCTCFD_row_filtered = df_QCTCFD[
                df_QCTCFD["VIDA_IN_Path"].str.contains(str(case_id), na=False)
            ]
            # df_row_to_append['Proj'] = df_QCTCFD_row_filtered['Proj'].values[0]
            df_row_to_append["Subj"] = df_QCTCFD_row_filtered["Subj"].values[0]
            df_row_to_append["Img"] = (
                "IN" + df_QCTCFD_row_filtered["FU"].astype("str").values[0]
            )
            df_row_to_append[
                "ImgDir"
            ] = f"{PATH_IN_REMOTE_HOST}/{OUTPUT_FOLDER}_{index}/{case_id}"
        elif (df_QCTCFD["VIDA_EX_Path"].str.contains(str(case_id))).any():
            df_QCTCFD_row_filtered = df_QCTCFD[
                df_QCTCFD["VIDA_EX_Path"].str.contains(str(case_id), na=False)
            ]
            # df_row_to_append['Proj'] = df_QCTCFD_row_filtered['Proj'].values[0]
            df_row_to_append["Subj"] = df_QCTCFD_row_filtered["Subj"].values[0]
            df_row_to_append["Img"] = (
                "EX" + df_QCTCFD_row_filtered["FU"].astype("str").values[0]
            )
            df_row_to_append[
                "ImgDir"
            ] = f"{PATH_IN_REMOTE_HOST}/{OUTPUT_FOLDER}_{index}/{case_id}"
    ...
    with open(f"{OUTPUT_PATH}/ProjSubjList.in.{today}_T1_{index}", "w") as f:
        f.write(df_to_write)

    return f"{OUTPUT_PATH}/ProjSubjList.in.{today}_T1_{index}"
